...
def lin_reg(X_tr, y_tr, X_test = None, y_test = None):
    ...
    if X_test == None:
        ...
        return(reg)
    reg.fit(X_tr, y_tr)
    predict_vals = reg.predict(X_test, y_test)
...
def elastic_reg(X_tr, y_tr, alpha_values, l1_ratio = [.1, .5, .7, .9, .95, .99, 1],
                X_test = None, y_test = None, m_folds = 10, fit_intercept = True , normalize = False):
    """
    ...
    fit_elastic = ElasticNetCV(l1_ratio = l1_ratio, alphas=alpha_values,
                               fit_intercept = fit_intercept, normalize = normalize, cv = m_folds)
    if X_test == None:
